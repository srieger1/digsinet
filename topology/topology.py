...
class TopologyBuilder:
    ...
    def add_link(self, node_from: str, node_to: str, interface_from: str, interface_to: str) -> None:
        # assert
        assert any(node.name == node_from for node in self._topo.nodes)
        assert any(node.name == node_to for node in self._topo.nodes)
        self._topo.links.append(Link(node_from, node_to, interface_from, interface_to))
