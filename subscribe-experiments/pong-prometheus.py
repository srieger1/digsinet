...
g = Gauge('gnmi_ping', 'Ping metrics', ['type'])
            ...
            if telemetry_data.get("update").get("update"):
                ...
                if path == "interfaces/interface[name=Ethernet1]/config/description":
                    ...
                    if val_components[0] == "gNMI" and val_components[1] == "PING":
                        ...
                        rtt = (current_time - float(val_components[2]))
                        #print("Ping received on path: " + path + " with data: " + val + " RTT: " + str(rtt))
                        # print current time, received time, and RTT in seconds
                        rtt_ewma = 0.9 * rtt_ewma + 0.1 * rtt
                        print("current: " + str(current_time) + " received: " + str(val_components[2]) + " RTT: " + str(rtt) + " RTT avg: " + str(rtt_ewma))
                        # send data to prometheus
                        g.labels("current").set(current_time)
                        g.labels("received").set(float(val_components[2]))
                        g.labels("rtt").set(rtt)
                        g.labels("rtt_ewma").set(rtt_ewma)
                        g.labels("ping").inc()
                    else:
